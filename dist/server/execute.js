...
export const HERMES_RETRY_DELAYS_MS = [2_000, 4_000, 8_000];
...
function extractRetryableStatusCode(text) {
...
export function getRetryableHermesStatusCode(result) {
    const failed = Boolean(result.timedOut) ||
...
export async function runHermesCommandWithRetries(options) {
    const { run, onRetryLog = () => undefined, sleep = wait, retryDelaysMs = HERMES_RETRY_DELAYS_MS, } = options;
    ...
    for (let retryIndex = 0; retryIndex < retryDelaysMs.length; retryIndex += 1) {
        ...
        await onRetryLog(`[hermes] Retryable provider failure detected (HTTP ${statusCode}). Retry ${attempt}/${retryDelaysMs.length} in ${Math.round(delayMs / 1000)}s.\n`);
        await sleep(delayMs);
        result = await run();
    }
    const exhaustedStatusCode = getRetryableHermesStatusCode(result);
    if (exhaustedStatusCode) {
        await onRetryLog(`[hermes] Retryable provider failure persisted after ${retryDelaysMs.length} retries (last status HTTP ${exhaustedStatusCode}). Surfacing final failure.\n`);
    }
    return result;
}
// ---------------------------------------------------------------------------
